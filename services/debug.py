...
async def debug_cleanup_legacy(
    ...
    _LOGGER.info(
        "DEBUG: Starting legacy cleanup - dry_only=%s, cure_only=%s",
        dry_only,
        cure_only,
    )
    ...
    try:
        ...
        if not cure_only:
            for legacy_id in legacy_dry:
                ...
                _LOGGER.debug(
                    "DEBUG: Migrating %d plants from legacy dry growspace %s",
                ...
                for plant in plants_to_migrate:
                    ...
                    # Assuming coordinator.plants is a dict of dicts or similar mutable structure
                    if plant_id in coordinator.plants:
                        ...
                        try:
                            ...
                            migrated_plants_info.append(
                                f"{plant.strain} ({plant_id}) to {canonical_dry} at ({new_row},{new_col})"
                            )
                ...
                _LOGGER.info("DEBUG: Removed legacy dry growspace %s.", legacy_id)
        ...
        if not dry_only:
            for legacy_id in legacy_cure:
                ...
                plants_to_migrate = coordinator.get_growspace_plants(legacy_id)
                _LOGGER.debug(
                    "DEBUG: Migrating %d plants from legacy cure growspace %s",
                    len(plants_to_migrate),
                    legacy_id,
                )
                ...
                for plant in plants_to_migrate:
                    ...
                    if plant_id in coordinator.plants:
                        ...
                        try:
                            ...
                            coordinator.plants[plant_id]["col"] = new_col
                            migrated_plants_info.append(
                                f"{plant.strain} ({plant_id}) to {canonical_cure} at ({new_row},{new_col})"
                            )
                        except Exception as e:
                            _LOGGER.exception(
                ...
                coordinator.growspaces.pop(legacy_id, None)
                removed_growspaces.append(legacy_id)
                _LOGGER.info("DEBUG: Removed legacy cure growspace %s.", legacy_id)
        ...
        _LOGGER.info(
            "DEBUG: Cleanup complete - removed %d growspaces. Migrated plants: %s",
...
async def debug_list_growspaces(
    ...
    if not coordinator.growspaces:
        _LOGGER.info("DEBUG: No growspaces found.")
        return
    ...
    for gs_id in coordinator.growspaces:
        ...
        if plants:
            ...
            _LOGGER.info("DEBUG: %s has 0 plants.", gs_id)
    ...
    try:
        if reset_dry:
            ...
            if preserve_plants and dry_plants_data_to_restore:
                restored_count = 0
                for plant_data in dry_plants_data_to_restore:
                    ...
                    if plant_id in coordinator.plants:
                        try:
                            ...
                            restored_count += 1
                            _LOGGER.debug(
                                "DEBUG: Restored %s to %s at (%d,%d) from %s",
                                plant_id,
                                canonical_dry,
                                new_row,
                                new_col,
                ...
                _LOGGER.info(
                    "DEBUG: Restored %d plants to canonical dry", restored_count
                )
        ...
        if reset_cure:
            ...
            if preserve_plants and cure_plants_data_to_restore:
                restored_count = 0
                for plant_data in cure_plants_data_to_restore:
                    plant_id = plant_data["plant_id"]
                    if plant_id in coordinator.plants:
                        try:
                            new_row, new_col = (
                ...
                _LOGGER.info(
                    "DEBUG: Restored %d plants to canonical cure", restored_count
                )
...
async def debug_consolidate_duplicate_special(
    ...
    try:
        ...
        _LOGGER.info("DEBUG: Found cure growspaces: %s", list(cure_growspaces.keys()))
            ...
            for duplicate_id in duplicate_ids:
                ...
                for plant in plants:
                    ...
                    if plant_id in coordinator.plants:
                        ...
                        _LOGGER.warning(
                            "Plant %s to migrate not found in coordinator.plants",
                            plant_id,
                        )

                coordinator.growspaces.pop(duplicate_id, None)
                _LOGGER.info("DEBUG: Removed duplicate dry growspace %s", duplicate_id)
            _LOGGER.info(
                "DEBUG: Dry consolidation complete - migrated %d plants", total_migrated
            )
        ...
        if len(cure_growspaces) > 1:
            ...
            for duplicate_id in duplicate_ids:
                ...
                coordinator.growspaces.pop(duplicate_id, None)
                _LOGGER.info("DEBUG: Removed duplicate cure growspace %s", duplicate_id)
            _LOGGER.info(
