...
In Volume Mode, total substrate volume — the basis for percent-of-volume shot sizing — is _liters per pot × the growspace's live plant count_, not a statically configured pot count or total. This makes shot sizing a **constant per-plant dose**: harvesting 6 of 12 plants halves the total shot volume automatically while each remaining plant keeps receiving the same dose.
