...
2. **The cap/limit sub-check is exported separately** as `safety_cap_blocks` and called _internally_ by `decide_cycle`. The coordinator's `_check_safety_guards` becomes a thin delegator to `safety_cap_blocks`, so Adaptive Shot Control's existing `capped` probe and `decide_cycle` share one definition — two consumers, one seam.
...
- **One place owns _why a cycle is skipped_.** The precedence (low-tank ≻ cycle-limit ≻ volume-cap ≻ dark), the manual-bypasses-dark rule, and the drain-ignores-cap/dark rule become an exhaustive plain-data test matrix asserted without `hass`, replacing reliance on the integration test for these branches.
...
- **Honest scope:** this deepens the _decision_, not the whole method. The counter mutation, switch-confirm timing, water recording, and `CancelledError`/error handling stay in the effectful shell and remain integration-tested.
