...
| `irrigation_tanks`     | `object`  | No       | -       | Irrigation nutrient tank volume & EC configurations. Each tank may set `stale_after_minutes` (default 120): how long its sensor may go without reporting before its level counts as unknown. |
...
tank's `stale_after_minutes` (default 120), or when it reads outside 0–100 %.
