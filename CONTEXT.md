...
**Crop Steering Score**
The absolute −1…+1 measurement of how generatively the substrate is behaving, computed by `calculate_crop_steering_score` from [[Dryback]], the [[EC Trend]], shot frequency, and (ADR-0016) a bounded, `None`-safe runoff component built from the sustained [[Feed-to-Runoff EC Delta]] (primary axis) with a small [[Runoff Percentage]] trim, clamped to ±0.3 so runoff never dominates dryback. The exact thresholds/nudges are an open HITL gate deferred to the implementation PR (only the structure and the ±0.3 budget are fixed). Stays a *measurement* — it never bends toward the declared [[Steering Mode]]; the [[Measured Classification]] and [[Intent Deviation]] are derived from it. The runoff component reads a config-thresholded physical signal (salts past the grower's `max_ec_delta`), the same measurement-relative-to-a-setting shape as [[Intent Deviation]] — not the score bending toward an intent.
