...
The value one steering tick returns, in the [[Cycle Verdict]] mould: it records the decision and performs none of the effects. Fields: `phase` (display string), `canonical` (`p1`/`p2`/`p3`, `None` for non-canonical states), `phase_changed`, a pure-formatted `transition_message`, `reset_composer` (the P1→P2 event — applied by the shell *before* any shot composes), `fire` (a `ShotRequest` naming the phase pair and the pre-composition **base** seconds; the [[ShotComposer]] and safety caps still apply downstream), and `volume_change_note` (the pure-formatted ADR-0011 logbook text). The shell maps fields to effects: `active_steering_phase`/`phase_changed_at` writes, the composer reset, logbook events (gated on `log_to_logbook` in the shell), and the pump cycle.
