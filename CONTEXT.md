...
**Strategy Stamp**
The shared write-and-record seam in `services/strategy_stamp.py` that every preset stamp goes through. A source (today the [[Steering Mode]] preset table) resolves a flat mapping of strategy field names → values; the seam does everything after that, identically for every source: write the values into the ordinary editable strategy fields, write the `records` naming which source was stamped (provenance the control loop never reads — the declared steering mode is the first), fire one logbook entry when the growspace has logbook entries enabled, invalidate the cache, commit and refresh. Stamps **always write**: the seam never diffs and never skips, so re-stamping is a deliberate "reset to these values" that discards hand tweaks (ADR-0012). Distinct from the sparse Irrigation Change seam (`services/irrigation_change.py`), which is the validated, grower-facing edit path.
...
**Recipe Stamp**
Applying an [[Irrigation Recipe]] writes its values into the ordinary editable strategy fields once and records `applied_recipe_id` + `recipe_applied_at`; the coordinator only ever reads the explicit fields, never the recipe. Identical semantics to the [[Steering Mode]] stamp (ADR-0012), including that applying **always writes** — re-applying the recipe already applied re-stamps, which doubles as "reset to this recipe" after hand-tuning. `applied_recipe_id` is nullable and `None` means "never applied", a real third state. No drift hash is stored: because recipes are held by reference, "has the grower tweaked since applying?" is a live field comparison computed on read. Each stamp writes one logbook entry naming the recipe and both media.
