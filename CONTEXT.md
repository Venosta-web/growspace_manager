...
**Irrigation Supply**
The one output per growspace that moves water into its [[Irrigation Zone]]s — a pump, or a master valve on pressurised mains. Only one zone is ever open on it at a time, so a flow meter on the supply line always measures exactly the one open zone. A grower with two feeds has two growspaces. Zones due at once take turns on it through its [[Supply Queue]]. See ADR-0058.
...
**Supply Queue**
The one queue per [[Irrigation Supply]] in which [[Irrigation Zone]]s that are due wait their turn, because only one zone is ever open on the supply. It serves [[Supply Claim]]s first in, first out by the minute each zone became due, ties in the grower's zone order, and puts a [[Manual Run]] at the front. It never interrupts a running shot and never holds a claim back for a reason of its own. It lives in memory only, and nothing in it survives a restart. See ADR-0061.
_Avoid_: pump queue (the supply may be a master valve), shot queue (what waits is not a composed shot).

**Supply Claim**
A zone's standing request for the [[Irrigation Supply]]: it says only that the zone is due, and a zone holds at most one. When it reaches the front of the [[Supply Queue]], the zone decides again from the present — the shot is composed, and the [[Infiltration Gate]] and [[Pump Cycle Gate]] are checked, only then — so a claim needs no deadline of its own, and one the zone no longer wants simply produces no shot. See ADR-0061.
_Avoid_: queued shot, pending attempt (a [[Delivery Attempt]] begins only at the front).
...
**Control Probe**
The one substrate moisture probe per [[Irrigation Zone]] that the grower elects, at a plant of average moisture, as the zone's steering figure; every other moisture probe is a **Witness Probe** — health evidence, display and a possible substitute, never averaged in. Probes are never combined because each sits on its own baseline: a mean describes no real pot and jumps whenever a probe drops out. Besides the [[Control Input]] causes, a probe is **unresponsive** after three consecutive confirmed steering shots without a rise above the infiltration deadband (Manual Runs and shots near saturation do not count), and recovers on one shot that does rise. When the Control Probe is unhealthy, a healthy witness with a learned offset — the median control-minus-witness difference over the last 24 h both were valid — may **substitute**, marked as such, with learning and dryback recording paused. See ADR-0059.
_Avoid_: main sensor, primary probe, average probe; zone VWC as a mean.

**Control Measurement**
The value steering acts on: a validated [[Control Input]] reading in % VWC with when it was reported, why it is invalid if it is, the validity window used, and which probe produced it — entity, role and position label, and `substitute_for` when a Witness Probe stands in for the [[Control Probe]]. Produced by the pure `domain/control_measurement.py` from a zone's probes. There is never a zero standing in for no measurement: without one the zone is in [[Degraded Control]]. See ADR-0059.

**Degraded Control**
A zone with no trustworthy [[Control Measurement]]: its [[Control Probe]] is invalid or unresponsive and no witness can substitute. VWC-triggered shots, Adaptive Shot Control and substrate event recording stop (the gap is recorded as a gap); Manual Runs and every gate continue. It reports as the zone's inhibit — the [[Control Input]] causes plus `probe_unresponsive` — withheld at once and alerted after `sensor_alert_delay_minutes` (ADR-0051). What replaces the withheld shots is the map's fallback item; until then, hold. A single-probe zone can only ever degrade, never silently switch. See ADR-0059.
_Avoid_: sensor fault (a [[Fault]] is a latched hardware disagreement; this clears with its cause).
