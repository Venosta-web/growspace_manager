...
**Effective Date**
The caller-supplied date a Lifecycle Transition is recorded as having occurred, passed as the `transition_date` parameter to `transition_plant_stage`, `take_clone`, `move_clone`, and `harvest_plant`. Defaults to now when omitted. Feeds directly into the written [[Lifecycle Timestamp]] via `to_lifecycle_timestamp()`. Distinct from Observed Date — the moment the transition _call itself_ was made — which has no explicit representation in the codebase today; until it does, it stays undocumented rather than naming a concept nothing implements.
_Avoid_: transition date (collides with the unrelated, now-removed `Plant.transition_date` field, which was dead scaffolding for an unwired auto-harvest scheduler)
