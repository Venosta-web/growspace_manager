...
**In-flight Marker**
The durable note in [[Reliability Evidence]] that a pump is running a cycle, written the moment it reads ON and cleared when the cycle closes. A marker still present at a start means the previous process stopped mid-cycle, and is counted as `runtime.ha_start_inflight`. It is evidence only; it switches nothing off. Whether a pump found ON at a start is Growspace Manager's own is answered by an open [[Delivery Attempt]], not by this marker (ADR-0055).
