...
**Strategy Stamp**
The shared write-and-record seam in `services/strategy_stamp.py` that every preset stamp goes through. A source (the [[Steering Mode]] preset table, or an [[Irrigation Recipe]] through the [[Recipe Stamp]]) resolves a flat mapping of field names → values; the seam does everything after that, identically for every source: write the values into the ordinary editable strategy fields, write `config_values` into the growspace's `IrrigationConfig` for the setpoints that live there instead (a schedule recipe's times, durations and caps — the same gesture writing the same growspace's irrigation settings, so it shares the seam rather than splitting it by which model holds the field), write the `records` naming which source was stamped (provenance the control loop never reads — the declared steering mode is the first, the applied recipe and its timestamp the second), fire one logbook entry when the growspace has logbook entries enabled, invalidate the cache, commit and refresh. Stamps **always write**: the seam never diffs and never skips, so re-stamping is a deliberate "reset to these values" that discards hand tweaks (ADR-0012). Distinct from the sparse Irrigation Change seam (`services/irrigation_change.py`), which is the validated, grower-facing edit path.
