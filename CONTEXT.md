...
**Strategy Stamp**
The legacy write-and-record operation retained by automatic [[Program Progression]] while its effects await migration. Explicit [[Recipe Stamp]]s and [[Steering Mode]] stamps use [[Irrigation Change]]; the automatic path shares their recipe candidate validation but retains its earlier persistence and narration behaviour (ADR-0046).

**Irrigation Change**
A validated change to a growspace's irrigation configuration, including explicit [[Recipe Stamp]]s and [[Steering Mode]] stamps. A refused candidate changes nothing; a raised commit restores the prior in-memory irrigation state and provenance, with success narration only after commit, without promising durable rollback across stores.
