...
class EnvironmentConfigHandler(BaseConfigHandler[dict[str, Any]]):
    ...
    async def _determine_next_step(
        ...
        if merged.get(CONF_CONFIGURE_FAN_CONTROLLER):
            return await self.async_step_configure_fan_controller()
    ...
    async def async_step_configure_fan_controller(
        ...
        if user_input is not None:
            ...
            if regulation_mode == FanRegulationMode.VPD:
                return await self.async_step_configure_fan_vpd()
            if regulation_mode == FanRegulationMode.HUMIDITY:
                return await self.async_step_configure_fan_humidity()
            return await self.async_step_configure_fan_temperature()
    ...
    async def async_step_configure_fan_vpd(
        ...
        if user_input is not None:
            ...
            if self.flow.fan_config_step1.get("wind_enabled"):
                return await self.async_step_configure_fan_wind()
    ...
    def get_fan_vpd_schema(self, current_cfg: CirculationFanConfig) -> vol.Schema:
        ...
        return vol.Schema(
            {
                ...
                vol.Optional(
                    "critical_temp_low",
                    description={"suggested_value": current_cfg.critical_temp_low},
