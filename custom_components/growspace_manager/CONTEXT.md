...
The state where an [[AC Infinity Device]] port's Active Mode `select` is in one of the controller's self-running modes (`Auto`, `VPD`, `Timer to On`, `Timer to Off`, `Cycle`, `Schedule`) — anything other than `Off`/`On` — at the time it is bound to a GSM role. In these modes the AC Infinity controller/cloud runs its own logic and re-asserts the port, so GSM's `On`/`Off` writes do not reliably stick; this is a limitation of the `ac_infinity` HACS component, not of GSM. GSM cannot resolve it (the setting lives on the device/app), so the card's config dialog surfaces a **passive, non-blocking warning** whenever a bound mode `select` reads a non-`Off`/`On` state — computed as a live derived value so it also appears on dialog reopen for a port left in an automated mode. `unavailable`/`unknown` are treated as "not an automated mode" (no warning), matching `ACInfinityDriver.is_on()`.
