...
def coordinator() -> MagicMock:
    ...
    mock.services.log_drying_weight = AsyncMock()
    mock.services.log_moisture_reading = AsyncMock()
    mock.services.set_visual_tag = AsyncMock()
    return mock
...
async def test_handle_log_drying_weight_all_fields(coordinator: MagicMock) -> None:
    ...
    coordinator.services.log_drying_weight.assert_awaited_once_with(
        plant_id="p1",
        weight_grams=320.5,
        date="2026-05-16",
    )
...
async def test_handle_log_moisture_reading_all_fields(coordinator: MagicMock) -> None:
    ...
    coordinator.services.log_moisture_reading.assert_awaited_once_with(
        plant_id="p2",
...
async def test_handle_set_visual_tag(coordinator: MagicMock) -> None:
    ...
    coordinator.services.set_visual_tag.assert_awaited_once_with(
        plant_id="p3",
