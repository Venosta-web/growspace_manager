...
def _make_coordinator() -> MagicMock:
    ...
    coordinator._nutrient_manager.inventory_service = MagicMock()
...
def test_get_inventory_delegates_to_inventory_service() -> None:
    ...
    coordinator._nutrient_manager.inventory_service.get_inventory.assert_called_once_with()
...
def test_get_inventory_returns_none_when_no_inventory_service() -> None:
    ...
    coordinator = _make_coordinator()
    coordinator._nutrient_manager.inventory_service = None
    facade = ConfigFacade(coordinator)
...
def test_update_stock_delegates_to_inventory_service() -> None:
    ...
    coordinator._nutrient_manager.inventory_service.update_stock.assert_called_once_with(
        nutrient_id="n1",
        name="Grow A",
        current_ml=500.0,
        initial_ml=1000.0,
...
def test_remove_stock_delegates_to_inventory_service() -> None:
    ...
    coordinator._nutrient_manager.inventory_service.remove_stock.assert_called_once_with(
        "n1"
    )
