...
def plant_service(
    ...
    return PlantService(
        ...
        growspace_service=mock_growspace_service,
...
async def test_take_clones_growspace_full(
    ...
    with pytest.raises(ValueError, match=f"Growspace '{clone_gs_id}' is full"):
        await plant_service.take_clones(
            ...
            num_clones=1,
            target_growspace_id=clone_gs_id
        )
...
async def test_take_clones_explicit_target_growspace_not_found(
    ...
    with pytest.raises(ValueError, match=f"Target growspace '{target_gs_id}' does not exist"):
        await plant_service.take_clones(
            mother_plant_id=mother_id,
            num_clones=1,
