...
def _make_coordinator(seed_batches: dict | None = None) -> MagicMock:
    ...
    coordinator._genetics_manager.seed_batches = seed_batches or {}
    coordinator._genetics_manager.get_total_seed_count = MagicMock(return_value=0)
    coordinator._genetics_manager.get_lineage_tree = MagicMock(return_value={})
    coordinator._genetics_manager.get_serialization_data = MagicMock(return_value={})
    coordinator._genetics_manager.async_add_seed_batch = AsyncMock()
    coordinator._genetics_manager.async_update_seed_batch = AsyncMock()
    coordinator._genetics_manager.async_log_pollination = AsyncMock()
    coordinator._genetics_manager.async_score_phenotype = AsyncMock()
    coordinator._genetics_manager.async_harvest_seeds = AsyncMock()
    coordinator._genetics_manager.async_update_pollination = AsyncMock()
    coordinator._genetics_manager.async_delete_pollination = AsyncMock()
    coordinator._genetics_manager.async_sow_seed = AsyncMock()
    coordinator._genetics_manager.async_set_plant_sex = AsyncMock()
    coordinator._genetics_manager.async_unlink_seed_batch = AsyncMock()
    return coordinator
...
def test_get_total_seed_count_delegates() -> None:
    ...
    coordinator = _make_coordinator()
    coordinator._genetics_manager.get_total_seed_count.return_value = 42
    facade = GeneticsFacade(coordinator)

    assert facade.get_total_seed_count() == 42
    coordinator._genetics_manager.get_total_seed_count.assert_called_once()
...
def test_get_lineage_tree_delegates() -> None:
    ...
    tree = {"nodes": []}
    coordinator = _make_coordinator()
    coordinator._genetics_manager.get_lineage_tree.return_value = tree
    facade = GeneticsFacade(coordinator)

    result = facade.get_lineage_tree("plant-1")

    assert result is tree
    coordinator._genetics_manager.get_lineage_tree.assert_called_once_with("plant-1")
...
def test_get_serialization_data_delegates() -> None:
    ...
    data = {"seed_batches": {}}
    coordinator = _make_coordinator()
    coordinator._genetics_manager.get_serialization_data.return_value = data
    facade = GeneticsFacade(coordinator)
...
async def test_async_add_seed_batch_delegates() -> None:
    ...
    coordinator._genetics_manager.async_add_seed_batch.assert_awaited_once_with(
...
async def test_async_update_seed_batch_delegates() -> None:
    ...
    coordinator._genetics_manager.async_update_seed_batch.assert_awaited_once_with(
...
async def test_async_log_pollination_delegates() -> None:
    ...
    coordinator._genetics_manager.async_log_pollination.assert_awaited_once_with(
...
async def test_async_score_phenotype_delegates() -> None:
    ...
    coordinator._genetics_manager.async_score_phenotype.assert_awaited_once_with(
        plant_id="p1", vigor=8
    )
...
async def test_async_harvest_seeds_delegates() -> None:
    ...
    coordinator._genetics_manager.async_harvest_seeds.assert_awaited_once_with(
...
async def test_async_update_pollination_delegates() -> None:
    ...
    coordinator._genetics_manager.async_update_pollination.assert_awaited_once_with(
...
async def test_async_sow_seed_delegates() -> None:
    ...
    coordinator._genetics_manager.async_sow_seed.assert_awaited_once_with("b1", "p1")
...
async def test_async_set_plant_sex_delegates() -> None:
    ...
    coordinator._genetics_manager.async_set_plant_sex.assert_awaited_once_with(
        "p1", "female"
    )
...
async def test_async_unlink_seed_batch_delegates() -> None:
    ...
    coordinator._genetics_manager.async_unlink_seed_batch.assert_awaited_once_with("p1")
