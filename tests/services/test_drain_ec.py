...
def coordinator():
    ...
    mock.services.growspaces.log_drain_reading = AsyncMock()
    mock.services.growspaces.configure_drain_monitoring = AsyncMock()
    return mock
...
async def test_handle_log_drain_reading_all_fields(coordinator) -> None:
    ...
    coordinator.services.growspaces.log_drain_reading.assert_awaited_once_with(
        growspace_id="tent1",
        feed_ec=2.0,
        drain_ec=2.4,
...
async def test_handle_configure_drain_monitoring_all_fields(coordinator) -> None:
    ...
    coordinator.services.growspaces.configure_drain_monitoring.assert_awaited_once_with(
        growspace_id="tent1",
        enabled=True,
        max_ec_delta=0.5,
