...
def mock_hass():
    ...
    # Just mock it, don't run immediately via side_effect because the caller is sync
...
async def test_handle_coordinator_update_add_growspace(
    ...
    # Manually await the task created by the listener
    mock_hass.async_create_task.assert_called_once()
    task = mock_hass.async_create_task.call_args[0][0]
    await task
